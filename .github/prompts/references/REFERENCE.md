...
This folder contains reference artifacts for the `drawio` skill.
...
# Fetches icon paths from the jgraph/drawio GitHub repository tree
...
for item in data.get('tree', []):
    p = item['path']
    if p.startswith('src/main/webapp/img/lib/azure2/') and p.endswith('.svg'):
        print(p.replace('src/main/webapp/img/lib/azure2/', ''))
" | sort > references/azure2-complete-catalog.txt
```
...
```text
Create a professional Azure network topology diagram emphasizing network isolation and traffic flows.

Requirements:
- Show VNet architecture with clear network boundaries (thick borders strokeWidth=4 for VNets, dashed borders strokeWidth=2 dashPattern=8 8 for subnets)
- Position all resources inside their respective subnets to show network isolation
- Label all traffic flows with protocols and ports (e.g., HTTPS:443, PostgreSQL:5432, HTTP:8080)
...
- Color-code different zones (DMZ VNet in yellow, Internal VNet in green, Management zone in blue, VNet Peering in grey, External Services in orange)
- Show VNet peering connections and external services in separate zones
- Use Azure2 icons from the verified icon catalog
```
